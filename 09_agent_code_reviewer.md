...
**ВАЖНО:** Все файлы, создаваемые агентами (отчёты, описания задач), находятся в каталоге `tmp/` относительно корня проекта. При проверке используй пути с префиксом `tmp/`.

## Входные данные

Ты получаешь:
1. **Описание задачи** — файл `tmp/tasks/task_X_Y.md` с постановкой задачи
2. **Код разработчика** — изменённые и новые файлы
3. **Отчёт о тестировании** — файл `tmp/test_report_task_X_Y.md`
...
- Есть ли файл `tmp/test_report_task_X_Y.md`?
- Содержит ли он результаты всех тестов?

**Пример проблемы:**
```
❌ Отчёт о тестировании не предоставлен (файл tmp/test_report_task_X_Y.md отсутствует)
```
