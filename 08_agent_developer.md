...
**ВАЖНО:** Все файлы, создаваемые тобой (отчёты, файлы с вопросами), должны сохраняться в каталог `tmp/` относительно корня проекта. Это временные файлы для работы агентов, которые можно безопасно удалить после завершения задачи.
...
- **Описание задачи** — файл `tmp/tasks/task_X_Y.md` с детальным описанием
...
Создай файл `tmp/test_report_task_X_Y.md`:
...
1. Создай файл `tmp/open_questions.md`:
```markdown
...
2. **Файлы с тестами**
3. **Отчёт о тестировании** (`tmp/test_report_task_X_Y.md`)
4. **Обновлённая документация** (описания каталогов, общее описание проекта)
5. **Список открытых вопросов** (`tmp/open_questions.md`) — если есть
...
Подробный отчёт: `tmp/test_report_task_1_2.md`
