...
**ВАЖНО:** Все файлы, создаваемые тобой, должны сохраняться в каталог `tmp/` относительно корня проекта. Это временные файлы для работы агентов, которые можно безопасно удалить после завершения задачи.

### 1. Создать низкоуровневый план разработки

Создай файл `tmp/plan.md` со следующей структурой:
...
- **Задача 1.1** — [Краткое описание]
  ...
  - Файл описания: `tmp/tasks/task_1_1.md`
  - Приоритет: Критичный
  - Зависимости: нет

- **Задача 1.2** — [Краткое описание]
  - Юзер-кейсы: UC-01
  - Файл описания: `tmp/tasks/task_1_2.md`
  - Приоритет: Высокий
...
Для каждой задачи создай отдельный файл `tmp/tasks/task_X_Y.md` со следующей структурой:
...
1. Создай файл `tmp/open_questions.md` со списком вопросов:
```markdown
...
1. **Файл `tmp/plan.md`** — общий план с последовательностью задач
2. **Файлы `tmp/tasks/task_X_Y.md`** — детальные описания каждой задачи
3. **Файл `tmp/open_questions.md`** — список открытых вопросов (если есть)
...
## Созданные файлы
- `tmp/plan.md` — общий план разработки
- `tmp/tasks/task_1_1.md` — описание задачи 1.1
- `tmp/tasks/task_1_2.md` — описание задачи 1.2
[...]

## Открытые вопросы
[Если есть — ссылка на файл `tmp/open_questions.md`]
